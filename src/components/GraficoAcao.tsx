...
export default function GraficoAcao({ acao }: Props) {
  ...
  function handleAtualizar() {
    setAtualizadoEm(new Date().toLocaleDateString("pt-BR"))
  }
    ...
    <div className="card-terminal">
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "1rem" }}>
        <span style={{ color: "#888", fontSize: "0.8rem" }}>
          Histórico intraday (simulado){atualizadoEm ? ` — atualizado às ${atualizadoEm}` : ""}
        </span>
        <button onClick={handleAtualizar} style={{ background: "#1a1a1a", border: "1px solid #333", color: "#e5e5e5", padding: "0.25rem 0.75rem", borderRadius: 4, cursor: "pointer", fontSize: "0.75rem" }}>
          Atualizar
        </button>
      </div>
